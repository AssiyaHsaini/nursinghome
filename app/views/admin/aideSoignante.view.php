...
<table class="table">
  <thead class="thead-dark">
    <tr>
      <th scope="col">Aide Soignante</th>
       <th scope="col">Service</th>
      <th scope="col">Pièce</th>
      <th scope="col">Tâche</th>
      <th scope="col">Description</th>
    ...
    foreach($this->data['common'] as $common)
        {
            ...
            <tr>
                    <th scope="row"><?= $common['lastname']."-".$common['firstname']; ?></th>
                    <td><?= $common['serviceName']; ?></td>                    
                    <td><?= $common['roomName']; ?></td>
                    <td><?= $common['taskName']; ?></td>
                    <td><?= $common['description']; ?></td>
                    <td><?= $common['expirationdate']; ?></td>

                    <td>
                    <?php 
                        if(!$common['did'] && $task['days']>=0)
                        { ?>
                            <form action="" method="POST">
                            <input type="hidden" name="postMethod" value="autres"> 
                                <input type="hidden" name="personId" value="<? echo $_SESSION["id"] ;?>"> 
                                <input type="hidden" name="taskId" value="<? echo $task['id_task'];?>"> 
                                <input type="hidden" name="roomId" value="<? echo $task['id_room'];?>">
                                <button type="submit" class="btn btn-dark btn-sm">Valider</button>
                        ...
                        else if(!$task['did'] && $task['days']<0)
                        {
                            ...
                            <form action="" method="POST">
                            <input type="hidden" name="postMethod" value="non">
                            <?  
                            echo  "<span class='badge badge-danger'>" . "Pôle emploie" . "</span>";
                        }
